...
def create_patch_heatmaps_indiv(patch, model256, output_dir, fname, threshold=0.5,
    ...
    if threshold != None:
        for i in range(6):
            ...
            Image.fromarray(region256_hm+img_inverse).save(os.path.join(output_dir, '%s_256th[%d].tif' % (fname, i)))
    ...
    for i in range(6):
        ...
        region256_hm = cv2.addWeighted(color_block256, alpha, save_region.copy(), 1-alpha, 0, save_region.copy())
        Image.fromarray(region256_hm).save(os.path.join(output_dir, '%s_256[%s].tif' % (fname, i)))
...
def create_patch_heatmaps_concat(patch, model256, output_dir, fname, threshold=0.5,
    ...
    if threshold != None:
        ...
        getConcatImage([getConcatImage(ths[0:3]), 
                        getConcatImage(ths[3:6])], how='vertical').save(os.path.join(output_dir, '%s_256th.tif' % (fname)))
    ...
    getConcatImage([getConcatImage(hms[0:3]), 
                    getConcatImage(hms[3:6])], how='vertical').save(os.path.join(output_dir, '%s_256hm.tif' % (fname)))
